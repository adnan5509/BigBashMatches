...
public class MatchDataProcessor implements ItemProcessor<MatchInput, Match> {
    ...
    public Match process(final MatchInput matchInput) throws Exception {
        ...
        String firstInningsTeam, seacondInningsTeam;
        if (matchInput.getToss_decision().equals("bat")) {
            firstInningsTeam = matchInput.getToss_winner();
            seacondInningsTeam = matchInput.getToss_winner().equals(matchInput.getTeam1()) ? matchInput.getTeam2()
                    : matchInput.getTeam1();
        } else {
            firstInningsTeam = matchInput.getToss_winner().equals(matchInput.getTeam1()) ? matchInput.getTeam2()
                    : matchInput.getTeam1();
            seacondInningsTeam = matchInput.getToss_winner();
        }
        match.setTeam1(firstInningsTeam);
        match.setTeam2(seacondInningsTeam);
